...
export function activate(context: vscode.ExtensionContext) {
    ...
    const disposable = vscode.commands.registerCommand("code-to-clipboard.copyCode", async () => {
        ...
        async () => {
            ...
            if (activeTextEditor) {
                ...
                if (isTextFile(filePath)) {
                    ...
                    const relativeFilePath = vscode.workspace.asRelativePath(filePath)
                    const extension = path.extname(filePath).slice(1)
                    outputChannel.appendLine(`[DEBUG] File: ${filePath}, Extension: ${extension}`)
...
export function detectAndDecodeFile(filePath: string): string | null {
    try {
        ...
        if (fallback) {
            outputChannel.appendLine(
                `[OK]  ${filePath}  ->  ${tried[tried.indexOf(detectedEnc)] ?? "utf-8"} (fallback)`
            )
            return fallback
        }
